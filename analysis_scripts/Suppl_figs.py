...
for i, dic in enumerate(list_dic[:]):
    ...
    T = conv_z.size
    allowed = f.allowed_circ_shifts(T, Hz_LB, period_sec=20.0, E_sec=6.0)
    rng = np.random.default_rng(0)
    my_shifts = rng.choice(allowed, size=10000, replace=True)
    r_null, shifts_used = f.circular_shift_null_corr_prestandardized(dffs_z, conv_z,n_shuffles=10000, shifts=my_shifts,batch_size=256,dtype=np.float32)
    pvals  = f.permutation_pvals_one_sided(all_coeffs, r_null, alternative="greater")
    
    # Sort the ROIs by correlation coefficients
