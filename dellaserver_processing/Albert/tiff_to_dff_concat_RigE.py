def main():
    ...
    parser.add_argument('--hpfsigma', dest='HPFSIGMA', type=int)
    ...
    if doConcatMoCo:
        vols_RigE.concat_volumes(green_moco_dir, greenpath, xdim=XDIM, ydim=YDIM, zdim=ZDIM-FLYBACKNUM)
        vols_RigE.concat_volumes(red_moco_dir, redpath, xdim=XDIM, ydim=YDIM, zdim=ZDIM-FLYBACKNUM)
    ...
    if doGetDFF:
        ...
        signal.getdff(greenpath, args.outdir, dff_name, shape=(XDIM, YDIM, ZDIM-FLYBACKNUM, NUMVOLS),F0window=args.F0WINDOW,HPF_sigma=args.HPFSIGMA)

    # plot mean green and red, annotate with args.flytrial
    if doPlot:
        NUMVOLS = len(io.get_file_paths(green_moco_dir))

        max_proj, mean_green, mean_red, mean_zdff = viz.quality_check(args.outdir, args.flytrial, fixed_brain_path, greenpath, redpath, zdffpath, shape=(XDIM, YDIM, ZDIM-FLYBACKNUM, NUMVOLS))
