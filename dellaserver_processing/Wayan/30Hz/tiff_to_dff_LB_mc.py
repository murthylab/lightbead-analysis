def main():
    ...
    from flydff import vols_LB, moco_LB, signal, viz #moco_g, 
    ...
    green_dir, red_dir = vols_LB._get_green_red_dirs(args.outdir)
    ...
    if doMoCo:
        if MOCO_CHANNEL == 1:
            moco_LB.apply(args.outdir, fixed_brain_path, start_image=start_image, end_image=end_image)
        #elif MOCO_CHANNEL == 0:
    ...
    """
    if doConcatMoCo:
        vols_LB.concat_volumes(green_moco_dir, greenpath, xdim=XDIM, ydim=YDIM, zdim=ZDIM)
        vols_LB.concat_volumes(red_moco_dir, redpath, xdim=XDIM, ydim=YDIM, zdim=ZDIM)
