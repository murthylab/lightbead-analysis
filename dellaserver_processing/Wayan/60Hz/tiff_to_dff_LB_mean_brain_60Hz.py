def main():
    ...
    if doGetVols:
        vols_LB_60Hz.get_from_tiffs(args.tiffdir, args.outdir)

    # save mean-tdtomato volume. Use later for moco and aligning to atlas
    if doMakeMean:
        vols_LB_60Hz.average_volume(args.outdir, ch=1, nvols=100, fname=fixed_brain)
        vols_LB_60Hz.average_volume(args.outdir, ch=0, nvols=100, fname=fixed_brain_green)
