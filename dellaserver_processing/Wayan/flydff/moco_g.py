...
def apply(outdir, fixed_brain_path):
    ...
    for ii in np.arange(NUMVOLS):
        ...
        moco_green = ants.registration(fixed, green, type_of_transform='Rigid')

        # apply transform to red volume
        moco_red = ants.apply_transforms(fixed, red, moco_green['fwdtransforms'])
        ...
        logging.info(f'saving {fpath}')
        ants.image_write(moco_red, os.path.join(red_moco_dir, fpath))
