...
from brainviz import io_green  # noqa: E402
...
def concat_volumes(indir, outpath, xdim=226, ydim=512, zdim=27):
    ...
    # initialize
    volnames = io_green.get_file_paths(indir)
    ...
    for ii, vol in enumerate(volnames):
        ...
        concat_vol[:, :, :, ii] = io_green.load(vol)
...
def average_volume(outdir, ch: int, nvols: int, fname=None):
    ...
    files = io_green.get_file_paths(vol_dir)
    ...
    io_green.save(output_path, mean_brain)
    logging.info("saved")
...
def _extract_volumes_from_tiffs(list_of_tiffs, green_dir: str):
    ...
    for TIFFIDX, tiff in enumerate(list_of_tiffs):
        ...
        for iPage in np.arange(STARTPAGE, NUMPAGES, 1):
            ...
            ch, frame, vol = io_green.decode_tiff_page_greenonly(THISPAGE, NUMZFRAMES)
            ...
            if ch == 0:  # ch=0 is green
                if frame != FLYBACKFRAME:
                    ...
                    io_green.save(os.path.join(green_dir, fpath), green_volume)
